...
documents and from issues. Audience IDs (`P1`–`P6`) refer to [Target Audiences](audiences.md).
...
| **A4** | **Raw MVT inspector**: click a tile, get layers → features → properties as a tree, with byte sizes per layer and a geometry preview | P3, P6    | `versatiles_geometry`, `probe -ddd` |
...
| **B5** | **Container diff**: compare two versions visually and statistically. A regression test for every rebuild                                                                          | P3, P6    | new                                            |
...
| **B8** | Glyph and sprite check: are glyphs missing for a given language? does the style reference an icon that does not exist?                                                            | P5, P6    | `versatiles-glyphs-rs`                         |
...
| **C4** | Parse and validation errors marked inline in the text at the correct position                                                                                                         | P3        | parser carries positions                                                                       |
...
| **C7** | **Export as CLI command, `.vpl` file, Dockerfile or GitHub Action snippet** — the bridge from desktop to production. Makes Studio an authoring tool for pipelines that run on servers | P2, P3    | trivial once C1 exists                                                                         |
| **C8** | Watch mode: source file changes on disk → preview updates                                                                                                                             | P6        | new                                                                                            |
