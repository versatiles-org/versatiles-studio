...
Why the shape of Studio is what it is, by subject. Within a chapter the numbers run in order, so an
area reads as it developed and an entry that amends another comes after it.
...
A `Q` number is never reassigned, and a new decision keeps the next one wherever its subject puts it -
the numbers are identities, not an order to read in. Evidence for upstream claims lives in the
...
None. New questions get a `Q` number here, and move into the chapter they belong to once settled.
