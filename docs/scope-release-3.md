...
| Item     | Work                                                                                                                                                                                                                                                                                        | Feature        |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| **S7.1** | **A project per window.** A `Project` holding graphs, style, history, pin, project directory and root; a registry keyed by window label, created when a window first asks and dropped when it closes. Every command that touches project state takes the window it came from                | infrastructure |
| **S7.2** | **Mounts namespaced per window.** One embedded server still (Q16), but mount keys carry the window, so two projects each with a graph called `pipeline` stop mounting over each other — and a pinned preview stops being a single global `preview` mount. A closing window unmounts its own | infrastructure |
| **S7.3** | **A job list per project.** Jobs belong to the project that submitted them: the status bar lists that project's work, and `Lane::Latest` supersedes only within it — today one window's keystroke cancels another window's preview build                                                    | E7             |
...
being narrowed; it needs a sibling proving that two scopes do _not_ supersede each other.
...
the problem log. It wants a test at the level of the mount key rather than of the map.
