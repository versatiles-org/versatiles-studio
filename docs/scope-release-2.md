...
| **S6.8** | ~~A problem log a user can copy~~ — **done**; the core keeps a folded ring of what went wrong and appends every occurrence to `problems.jsonl` in the log directory, rotating the last run's file aside at launch so a window that crashed can still be read. The webview reports what only it can see — an uncaught rejection, a MapLibre `error` event, anything written to `console.error`, every `status.fail` — a panic hook catches the half that used to reach nobody, and `<svelte:boundary>` per pane keeps one of them failing from taking the window. The status bar expands into the list, with a tab for the previous run, and one button puts a report on the clipboard with the build, the platform and the GPU, and the home directory redacted out | infrastructure |
