...
| Item       | Work                                                                                                                                                                                                                    | Feature        |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| **S6.1**   | Surface `tile_schema` through `ContainerInfo` and say what a source is being drawn as, with a picker that overrides it. Probing stays the fallback for containers written before the field existed                      | infrastructure |
| **S6.2**   | Derive a style where a preset would draw nothing, instead of drawing none. The condition is already computed at `App.svelte:609`; `deriveStyle` already exists                                                          | D2             |
