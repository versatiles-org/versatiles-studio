...
| **S4.5**    | ~~Layer tree with filter/zoom/paint editing and an expression editor~~ — **done**: every layer of the rendered style, grouped by the tile layer it draws, filterable, with visibility, colour and zoom range going back into the recipe, and a filter editor that applies on every valid keystroke so the map answers the guess. The expression editor edits **filters**, not colours ([Q37](decisions.md)): no preset, no derived style and no import path produces a colour expression, while 1,475 of 1,503 layers carry a filter and every one is an expression. Validated by MapLibre's own `featureFilter` | D3             |
