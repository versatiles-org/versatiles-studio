...
| **S0.13** | The application's name wherever the system shows it. `productName` and the bundled `CFBundleName` are already **VersaTiles Studio**; `CFBundleExecutable` and the `cargo tauri dev` binary are `versatiles-studio`, which is what the dock and the app switcher fall back to. `mainBinaryName` settles both | infrastructure |
...
| **S1.11**   | A user agent naming Studio on every remote request. Two paths, and only one is ours: the webview's is `app.windows[].userAgent` in `tauri.conf.json`, while the Rust side is `versatiles_core::io::USER_AGENT` — a compile-time `const` with no way to override it, so this needs an upstream ask ([ecosystem](ecosystem.md#upstream-asks)) | infrastructure |
...
| **S2.13**   | ~~**The graph list and the node-as-form**~~ — **done**; pin, unsaved dot and inline rename in the list; a node shows only its name unless selected, the head keeps its filename, `＋ operation…` hangs off the rail. The Parameters pane is gone                                                                                        | C2             |
| **S2.14**   | Recommend the operations that fit, when appending a node. `compatible_transforms` and `Compatibility` landed in 4.9.0 ([vt#235](https://github.com/versatiles-org/versatiles-rs/issues/235), asked for by us) — the picker groups what fits above what does not, with the reason for the refusal                                        | C2             |
| **S2.15**   | Pretty-print the VPL document. `VPLPipeline::to_string_pretty` arrived in 4.9.0 but lowers through the AST, which **drops comments** — the one thing [Q23](decisions.md)'s lossless CST exists to keep. Needs a formatter that rewrites the CST's whitespace trivia and leaves the rest, which is an upstream ask before it is our code | C1             |
...
| **S4.3**    | Preset styles with global recolouring — the six `@versatiles/style` builders over the project's own graphs, and `RecolorOptions` exposed                                                                                                                                                                                                                                 | D1             |
...
| **S4.1**    | Asset manager: download, pin, verify and remove font families and sprite sets, plus the mode bar that reaches it — **Map · Assets** ([Q22](decisions.md))                                                                                                                                                                                                                | G7             |
...
| **S4.8\***  | Derive a dark variant from a light style — `RecolorOptions.invertBrightness`, so it is smaller than a stretch item suggests                                                                                                                                                                                                                                              | D5             |
...
**S4.4 is the hardest item in the release**, though less so than it looked. It cannot assume
Shortbread layers — it has to read what is in the container, which is the same introspection as A4
(S1.10), and that shipped. It is also the _fallback_: a container whose layers are Shortbread gets a
preset, and only one that is not reaches the deriver.
