...
| **S2.12**   | **Several named graphs per project** ([Q32](decisions.md)): the core holds a set of documents, one mount per graph, a global undo stack, and rename rewriting style references                                     | C1             |
| **S2.13**   | **The graph list and the node-as-form**: pin, unsaved dot, inline rename; a node shows only its name unless selected, then its arguments with `?` docs and the rail's `＋ operation…`. Retires the Parameters pane | C2             |
...
| **S3.6**   | Write the result to a container, as a `queued` job: a tile-count guard before the traversal starts, a zoom bound, and a per-graph export modal — format, zoom, numeric bounds, estimate ([Q32](decisions.md))                                                                                                                         | F2             |
