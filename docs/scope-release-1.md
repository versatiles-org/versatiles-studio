...
| **S4.6**    | ~~Export `style.json`, `@versatiles/style` code, or a bundle~~ — **the first two done**; both carry a tile-URL placeholder rather than the ephemeral local port the map reads from, and a derived style offers only the JSON, having no builder to name. A bundle waits on [S5.1](#work-items)'s project directory                                                                                        | D8             |
...
| Item     | Work                                                                                                                                                                                                                                                                                                                                                | Feature        |
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| **S5.1** | Project directory: `project.yaml` beside real `.vpl` and `style.json`; zip and "Save As" — **save and open done**: the manifest carries the graphs and the style _recipe_ ([Q36](decisions.md#q36--the-core-owns-the-styles-recipe-not-the-style)), and every file beside it is usable without Studio. Zip, and copying a project elsewhere, remain | G1             |
...
| **S5.5** | Export as CLI command, serve config, Dockerfile or GitHub Action — **generators done**: `deploy.rs` builds all four from the project as it stands, never stored, and the serve config is checked against `versatiles`' own parser rather than eyeballed. Showing and copying them is what remains                                                   | C7             |
