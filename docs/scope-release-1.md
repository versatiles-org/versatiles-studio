...
| **S2.16**   | Tile activity in the status bar: how many tiles are **queued** and how many are **rendering**, shown only once the wait passes a second. MapLibre's `addProtocol` puts the queue in Studio's hands — with our own concurrency limit at or below the browser's per-origin cap, in-flight means the server really has it, and everything else is ours to count. Neither half is visible otherwise: MapLibre's `sourcedataloading` fires when it _issues_ a fetch, not when one reaches the wire, and a counter inside the mounted `TileSource` would see only the handful the browser let through | C3             |
| **S2.15**   | Pretty-print the VPL document. `VPLPipeline::to_string_pretty` arrived in 4.9.0 but lowers through the AST, which **drops comments** — the one thing [Q23](decisions.md)'s lossless CST exists to keep. Needs a formatter that rewrites the CST's whitespace trivia and leaves the rest — filed as [vt#249](https://github.com/versatiles-org/versatiles-rs/issues/249)                                                                                                                                                                                                                         | C1             |
