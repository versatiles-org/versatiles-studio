...
| **S2.16**   | ~~Tile activity in the status bar: how many tiles are **queued** and how many are **rendering**, shown only once the wait passes a second~~ — **done**; `addProtocol` puts the queue in Studio's hands, with a limit of 6 to match the browser's per-origin cap so that in-flight means the server really has it. Background tiles are left out deliberately: they come from versatiles.org, and queueing them would report someone else's network as this pipeline being slow | C3             |
