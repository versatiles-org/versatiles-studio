...
| Item      | Work                                                                                                                                                                                                                                                       | Feature        |
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| **S0.1**  | ~~Tauri shell: one window per project, dialogs, drag & drop, file associations, native menus~~ — **done**; File, Help and the platform's own submenus, with ⌘N, ⌘O and ⌘S on them ([Q47](decisions.md)). The in-window application bar it replaces is gone | infrastructure |
...
| **S3.3**   | ~~Vector import: GeoJSON, NDJSON, shapefile~~ — **done**; no wizard — the imported node is selected, and the form offers the property names the data actually has ([Q29](decisions.md)). What the graph produces is reported in the export dialog rather than a pane of its own ([Q41](decisions.md)) | E1             |
...
| **S3.7**   | ~~Sampling-based cost estimate, shown where a run is committed~~ — **done**; the real pipeline over a spread of tiles under a two-second budget, stratified by zoom because that is where the variance is. Live in the crop section, on request in the export dialog ([Q42](decisions.md))            | C6             |
...
| **S5.2** | ~~Crop and estimate in the Pipeline pane; the map as a crop tool. No Export pane ([Q31](decisions.md))~~ — **done**; the crop lives on the graph in the core, so it survives a reload and goes into the manifest. Folded away by default ([Q43](decisions.md))                                                                  | infrastructure |
...
| **S5.5** | ~~Export as CLI command, serve config, Dockerfile or GitHub Action~~ — **built, then removed** ([Q40](decisions.md)): the four artefacts answered two different questions and two of them contradicted each other. `deploy.rs` and the dialog are gone                                                                          | ~~C7~~         |
...
| **S5.8** | ~~Auto-update~~ — **done**; checked when asked, never on a timer — an app that swaps itself out mid-export is what people turn updaters off to escape. Asked for from the menu and answered in a dialog ([Q47](decisions.md)). Only a **published** release resolves                                                            | G4             |
