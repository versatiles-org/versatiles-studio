...
| **S3.5**   | **Blocked upstream** — GDAL cannot link into Studio: `proj-sys` and `versatiles_container` need incompatible `libsqlite3-sys` versions of the same `links` key ([Q19](decisions.md)). The import card is written and appears the day it resolves | E3             |
