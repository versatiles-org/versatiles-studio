...
| **S5.6** | ~~Linux packaging: `.deb` plus an AppImage, from GitHub releases~~ — **done**; `release.yml` builds on a `v*` tag and attaches the installers to a **draft** release, because publishing is what reaches every installed copy and is a person's decision. A tag whose version disagrees with the files fails in ten seconds rather than producing three installers named 0.0.0                                                                                                                  | G3             |
