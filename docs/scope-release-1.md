...
| **S5.6** | ~~Linux packaging: `.deb` plus an AppImage, from GitHub releases~~ — **done**; `release.yml` builds on a `v*` tag and drafts a release, and `npm run release` drives the whole thing: refuse on a dirty tree, run every check, bump the four files that state the version, write the notes, tag, **ask once**, then push, wait, publish and fill in the cask. A tag whose version disagrees with the files fails in ten seconds rather than producing three installers named 0.0.0              | G3             |
