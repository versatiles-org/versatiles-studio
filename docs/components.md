...
Grouped by where a component appears on screen rather than by folder — the listing above answers the
other question. Styling, the tokens and what is enforced are in [Styling](styling.md); a component
that adds a map layer must tag it with `role()` from `lib/map/theme.ts` or the layer will not follow
the theme.

| Component          | Does                                                                                                      | Stage |
| ------------------ | --------------------------------------------------------------------------------------------------------- | ----- |
| `App`              | The workbench: one window, one project ([Q48](decisions.md))                                              | S0.1  |
| `Launcher`         | The launcher window's root. Every gesture ends in a project window opening and this one closing           | S7.5  |
...
| `Boundary`         | One pane or the map failing instead of the window; records what it caught and offers a retry              | S6.8  |
| `StatusBar`        | What the application is doing; progress, cancellation, and where errors land ([Q24](decisions.md))        | S1.9  |
| `JobsPanel`        | Every job **this project** has run, expanded upward from the bar; opens one job's log (E7, S7.3)          | S3.1  |
| `DiagnosticsPanel` | What has gone wrong, this session and the run before it, with a report to copy into an issue              | S6.8  |
| `AlphaRibbon`      | What state this is in, across the top-right corner, linking to the repository in the system browser       | —     |

## Map

One `Map` instance per window, and one project per window ([Q16](decisions.md), [Q48](decisions.md)).

| Component        | Does                                                                                                                                                                  | Stage |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| `MapCanvas`      | Wraps MapLibre; viewport restored from the core, never from local state. Styles are applied when the map is ready for one ([S7.4](scope-release-3.md))                | S1.4  |
| `MapControls`    | Background picker, grid toggle and Reset view, over the map                                                                                                           | S1.6  |
| `TileGrid`       | z/x/y grid (A5)                                                                                                                                                       | S1.7  |
| `TileActivity`   | Tiles the map is still waiting for, shaded and labelled `queued` or `rendering`                                                                                       | S2.16 |
...
| `GraphList`    | The project's graphs as a layers panel: the eye, name, how much of it runs, unsaved dot, inline rename ([Q49](decisions.md))                                                             | S2.2       |
| `NodeChain`    | The chain of nodes; vertical, with `＋ operation…` on the rail outside them ([Q32](decisions.md))                                                                                        | S2.13      |
| `NodeCard`     | One node in the chain: its eye, arguments, `?` docs and `×`. Every node shows all of it ([Q32](decisions.md), [Q49](decisions.md))                                                       | S2.6       |
...
| `StylePane`    | The source stack, the preset and the adjustments over it (D1); says which of four routes produced the style ([S6.2](scope-release-2.md))                                                 | S4.2       |
...
**The generated form carries the architecture, and it shipped as `NodeArgument`.** C2 generates forms
from `field_meta` rather than one per operation, so a single component covers all ~30 VPL operations,
upstream additions appear for free, and S3's import cards are just a `from_*` node's form. Planned as
a `ParamForm` over an `Input*` kit; built, the variants differed only in the data handed to them, so
it is one component and a `control.kind` switch. `is_sources` fields — which pick another node — are
the one control with no counterpart in any existing repo.
...
An errand you leave the window for and come back from ([Q39](decisions.md)).

| Component      | Does                                                                                                                                                                                  | Stage            |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| `Modal`        | The dialog shell: title, width, body, buttons. Used by all of them                                                                                                                    | S5.5             |
| `ExportDialog` | What the graph produces, what will be written, an estimate on request ([Q42](decisions.md)), then the file — per graph ([Q32](decisions.md), [Q41](decisions.md))                     | S3.3, S3.6, S3.7 |
| `CopyDialog`   | What a portable copy would carry, and whether to write it as a folder or a `.zip` (G1)                                                                                                | S5.1             |
| `AssetsDialog` | Font families: size, install, remove, download all. The bundled tier is not listed, being unremovable                                                                                 | S4.1             |
